...
4. **If a newer version exists**, proceed to Step 2.
...
3. **If tests fail:**
   ...
   - Check if the new OpenFisca Core version has breaking changes
   - Review the OpenFisca Core release notes/changelog
   - If the version is incompatible, stop here and report the issue
   - Consider if `COUNTRY_TEMPLATE_VERSION` also needs updating (check compatibility matrix in DEVELOPER_GUIDE.md)
...
1. Identify the image tag that was created. The workflow uses `docker/metadata-action` which creates tags based on:
   - Branch name: `update/openfisca-core-X.X.X`
...
2. Pull the new image from GHCR:
   ...
   ```
   (Replace with the actual tag created by the workflow)
...
4. **If verification fails**, investigate and report the issue.

5. **If verification succeeds**, proceed to Step 8.
...
1. Create a pull request from your feature branch to `main`:
   ...
   - Description should include:
     ...
     - Link to OpenFisca Core release notes/changelog (if available)
...
2. Example PR description:
   ...
   ## References
   - OpenFisca Core release notes: [link if available]
   ```
...
- Check OpenFisca Core release notes for breaking changes
- Verify Country Template compatibility
